package com.example.feature.screens.hub.home.sedouka
...
fun Sedouka(
    ...
    LazyColumn(
        ...
        items(
            homeState.sedoukas.size,
            key = { index: Int -> homeState.sedoukas[index].id },
        ) { i ->
            SedoukaCard(
                post = homeState.sedoukas[i],
                hubState = hubState,
        ...
        item {
            if (!homeState.isAllSedoukasFetched && homeState.sedoukas.isNotEmpty()) {
                Column {
                    Spacer(modifier = Modifier.padding(dimensionResource(R.dimen.padding_smaller)))
                    LoadingUI(
                        onLoading = {
                            if (homeState.sedoukas.isNotEmpty()) {
                                onHomeAction(
